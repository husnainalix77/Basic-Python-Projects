...
class Reversi:
    ...
    def isValidMove(self, user_move, mark, grid=None, simulateMove=False):
        ...
        if grid is None: ## if grid is not passed, use original grid
            grid = self.grid
        row = int(user_move[1:]) - 1
        col = self.letters.index(user_move[0])
        if grid[row][col] != '-':
            return False
    ...
    def isGameOver(self):
        ...
        if '-' not in [cell for row in self.grid for cell in row]:
            return True
        for mark in ['B', 'W']:
            if self.availableValidMoves(mark): ## if at least one valid move
                return False
        return True
