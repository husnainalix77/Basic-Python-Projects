...
def setUpGrid(n): # n is size of grid; 4 
    'Sets up grid when solvable grid is found'
    tiles = list(range(1, 16)) + ['-'] ## 1D List
    while True:
        random.shuffle(tiles) ## Each time random list for solvable grid
        ...
        for j in range(len(nums)):
            for k in range(j + 1, len(nums)):
                if nums[j] > nums[k]:
                    inversions += 1
        # Row containing Blank from bottom
        blank_tile = tiles.index('-')
        row_from_top = blank_tile // n
        blank_row_from_bottom = n - row_from_top ## In 15 Puzzle Game, Rows number starts from 1 
        # Check solvability
        if (inversions + blank_row_from_bottom) % 2 == 0:
            break
    # Convert to 2D grid
    grid = [tiles[r:r + n] for r in range(0, n * n, n)]
    return grid
