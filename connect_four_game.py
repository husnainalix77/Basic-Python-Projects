## Connect Four Game (Human vs AI) — Designed and Coded by Husnain Maroof, ML Engineer, on 6 Sep, 2025.
import random
import time
import os
import platform
...
def playGame(rows, cols, difficulty): 
    'Full Playable game with AI difficulty flexibility'
...
if __name__=='__main__':  
    ...
    while True:
        ...
        if op not in ['y', 'n']:
            ...
            continue
        if op == 'y':
            playGame(rows, cols, difficulty=0.6) ## AI : 60% strategic, 40% flexible
            print("="*40)
        else:
            print('Ok, next time.')
            break
